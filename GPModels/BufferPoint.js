...
Buffer.outputs["output_buffer"] = { wkt: "", srid: "" };
...
"SELECT ST_ASText(ST_transform(ST_Buffer(ST_transform(input, utm_srid), 2000), orig_srid));" +
"END$$;" +
"select * from _temp;"
...
Buffer.isInputValid = function (input) {
    ...
    if (input) {
        if (input["input_geometry"] && input["buffer_distance"]) {
            if (input["input_geometry"].wkt && input["buffer_distance"].value) {
...
Buffer.execute = flow.define(
    function (args) {
        ...
        if (Buffer.isInputValid(args) === true) {
            ...
            Buffer.executePgQuery(Buffer.bufferQuery.replace("{wkt}", Buffer.inputs["input_geometry"].wkt), this);//Flow to next function when done.
