...
exports.batchBlast = flow.define(
    ...
    function (exists) {
        ...
        blastConfig.sources.forEach(function (source) {
            //Execute call
            executeRESTRequest(source.options.tablename, source.options.postArgs, function (err, result) {
                ...
                transformData(result, source.options.transformer, function (err, transformed_data) {
                    ...
                    var jsonOutFile = source.options.tablename + '.json';
                    fs.writeFile("." + settings.application.jsonOutputFolder + "/" + jsonOutFile, JSON.stringify(transformed_data).replace(/\s+/g, ''), function (err) {
                        if (err) {
                        ...
                        if (receiveCount == blastConfig.sources.length) {
                            ...
                            //Write Index.js file
                            fs.writeFile("." + settings.application.jsonOutputFolder + "/index.js", "var _blasterindex = " + JSON.stringify(indexArray).replace(/\s+/g, '') + ";", function (err) {
                                if (err) {
                                    console.log(err.message);
                                }
                                else {
