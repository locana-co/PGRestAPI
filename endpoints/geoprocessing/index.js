...
app.all('/services/geoprocessing/geoprocessing_operation', flow.define(
...
function (req, res) {
    ...
    if (JSON.stringify(this.args) != '{}') {
        ...
        this.args.breadcrumbs = [{ link: "/services/tables", name: "Table Listing" }, { link: "/services/geoprocessing", name: "Geoprocessing Operations" }, { link: "", name: "Geoprocessing Operation" }];
        this.args.path = this.req.path;
        this.args.host = this.req.headers.host;
                
        if (this.args.name) {
            //Dynamically load the page
            this.gpOperation = gp.operations[this.args.name.toLowerCase()]; //always lower names.
            ...
            this.args.breadcrumbs = [{ link: "/services/tables", name: "Home" }, { link: "/services/geoprocessing", name: "Geoprocessing Operations" }, { link: "", name: this.gpOperation.name }];
            ...
            for (var key in this.gpOperation.inputs) {
                if (this.gpOperation.inputs.hasOwnProperty(key)) {
                    ...
                    if (this.args[key]) {
                        ...
                        this.args._input_values.push({ name: key, value: this.args[key] });
                    }
                }
            }
            ...
            if (this.args.formfields.length == this.args._input_arguments.length) {
                ...
                this.gpOperation.execute(this.args, this); //Flow to next bloc when done
  ...
  function(result){
        ...
        if (result.status == "error") {
            ...
            if (!this.args.format || this.args.format.toLowerCase() == "html") {
                //Render HTML page with results at bottom
                features = common.formatters.geoJSONFormatter(result.rows, this.args.geom_fields_array); //The page will parse the geoJson to make the HTMl
            }
