...
operation.execute = flow.define(
  function (args, callback) {
      ...
      if (operation.isInputValid(args) === true) {
          ...
          var query = { text: this.Query.replace("{where_clause}", operation.inputs["where_clause"]).replace("{buffer_distance}", operation.inputs["buffer_distance"]).split("{country}").join(countries[operation.inputs["country_code"]].name).replace("{srid}", countries[operation.inputs["country_code"]].srid), values: [] };
